...
"""
run_stage3_phaseA_full.py  (RESUME-SAFE for all parts + cache-in-output-dir, with LLM repair moved to Part 2)

This script now supports:
1) Stage 1 (term sampling -> terms.jsonl / terms_selected.jsonl) resume:
   - If terms.jsonl exists, it will:
       * load existing rows into memory
       * collect seed_hpo values that already appeared
       * skip those seeds when sampling new terms
       * append ONLY new terms to terms.jsonl
       * rebuild terms_selected.jsonl from all terms (old + new)
   - Stage 1 is now **purely local** (no ESearch / LLM).
2) Stage 2 (PubMed ESearch + ELink + query->PMCIDs) resume:
   - As before, based on query_to_pmcids.jsonl and cache
   - LLM-based query repair is now done **here**:
       * First run ESearch for all unique used_query
       * For queries with 0 PMIDs, optionally call LLM to repair the query
       * Run a second ESearch with the repaired query
       * Only these "truly 0-hit" queries call LLM
3) Stage 3 (PMC EFetch + PhaseA parse/extract/merge) resume:
   - As before, uses per-PMCID DONE markers and merged jsonls with append-only writes
"""
...
def find_anchors(ctx: str) -> List[str]:
    ...
    # NOTE: This literal pass is as in your file; left unchanged
    lit_keep = [
...
def _join_pmcids_from_pmid_map(pmids: List[str], pmid_to_pmcids: Dict[str, List[str]]) -> List[str]:
    ...
    for pmid in pmids:
        for pmcid in (pmid_to_pmcids.get(str(pmid)) or []):
            pmcid = _normalize_pmcid(pmccid)  # NOTE: kept as-is if this matches your local file (typo in original)
...
def main():
    ...
    if os.path.exists(selected_path):
        ...
        for si, seed_hpo in enumerate(pbar_seeds, start=1):
            ...
            def _handle_term(r: Optional[TermRecord], kind: str) -> None:
                ...
                # 🔁 NEW: Stage1 no longer calls maybe_repair / ESearch / LLM
                rr = r
        ...
        for d in terms_all:
            if args.check_esearch:
                # For backward compatibility, still respect used_hits>0 if populated
                uh = d.get("used_hits")
    ...
    # -----------------------------
    # Part 2: Query -> PMIDs/PMCIDs (RESUME SAFE, with query-level LLM repair)
    # -----------------------------
    ...
    if bad_queries and args.use_llm_repair and llm is not None:
        ...
        for k, q in enumerate(pbar_fix, start=1):
            ...
            try:
                ...
                # Try common LLMClient APIs; adapt if needed
                if hasattr(llm, "chat"):
                ...
                # Overwrite mapping for original query q
    ...
    # ---- Step 2.2 Rebuild unique PMIDs, then ELink ----
