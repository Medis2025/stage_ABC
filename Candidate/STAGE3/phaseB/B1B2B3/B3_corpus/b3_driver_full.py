...
# -*- coding: utf-8 -*-
"""
b3_driver_full.py  (PLAN A+++: set-level corpus item = one "patient phenotype set")

FULL BATCH PROCESSING VERSION
=============================
This is your b3_test_driver upgraded to a production-ish batch runner:

✅ Keeps your A+++ logic (anchors + gating + attempt2 required-keywords + attempt3 missing-repair)
✅ Adds "full processing batch" features:
   1) Streaming-safe incremental JSONL writing (no giant list in RAM)
   2) Resume-safe (skip already generated seed_i + signature)
   3) Optional DRY RUN (no LLM calls, just sampling + evidence/index + anchor preview)
   4) Batch mode with periodic flush + progress stats
   5) Rich debug printing + tqdm postfix
   6) Failure log (failures.jsonl) & minimal debug summary for failures
   7) Optional "write_only_gen_ok" corpus output
...
NOTE
----
This script assumes LLMClient has:
  - complete_text(system: str, user: str, temperature: float, max_tokens: int) -> str
...
def generate_set_with_gating(
    *,
    ...
    # DRY RUN: no API call. Return anchor preview + evidence size.
    if dry_run:
        dbg = {
            ...
            "seed_hpo": seed_hpo,
            "n_hpos": len(hpos),
            "note": "DRY_RUN: no LLM call, no lines generated",
        }
        return {
            "ok": False,
            "attempts": 0,
    ...
    if llm is None:
        raise RuntimeError("llm is None but dry_run=False")
...
def set_signature(hpos: List[str]) -> str:
    # stable-ish signature to dedupe across runs
    return "|".join(sorted([str(x) for x in hpos]))

def load_existing_signatures(corpus_jsonl: str) -> Set[str]:
    sigs: Set[str] = set()
    ...
    try:
        for obj in iter_jsonl(corpus_jsonl):
            hpos = obj.get("hpos", [])
            if isinstance(hpos, list) and hpos:
                sigs.add(set_signature([str(x) for x in hpos]))
    except Exception:
        # If the file is partially written/corrupt, ignore and continue
        pass
    return sigs
...
def main() -> None:
    ap = argparse.ArgumentParser("b3_driver_full (PLAN A+++: set-level corpus)")
    ...
    ap.add_argument("--per_seed_candidates", type=int, default=40)
    ...
    ap.add_argument("--min_hits_per_line", type=int, default=2)
    ap.add_argument("--must_include_seed", type=int, default=1)
    ap.add_argument("--require_full_coverage", type=int, default=0)
    ap.add_argument("--min_coverage_ratio", type=float, default=0.7)

    ap.add_argument("--required_keywords_show_per_hpo", type=int, default=2)

    ap.add_argument("--use_blacklist", type=int, default=1)
    ap.add_argument("--blacklist_extra", default="")

    # batch / resume / debug
    ap.add_argument("--dry_run", type=int, default=0, help="1=do not call LLM; just build indices + sample sets + show anchor previews")
    ap.add_argument("--resume", type=int, default=1, help="1=skip sets already present in corpus_generated.jsonl")
    ap.add_argument("--write_only_gen_ok", type=int, default=0, help="1=only write gen_ok items to corpus_generated.jsonl (still logs failures)")
    ap.add_argument("--flush_every", type=int, default=10, help="progress print every N items (jsonl is appended immediately anyway)")
    ap.add_argument("--debug_print_fail", type=int, default=1, help="1=print failure debug brief")
    ap.add_argument("--debug_print_every", type=int, default=0, help="if >0, print brief debug for every N items")
    ...
    print("[MODE] dry_run        =", bool(args.dry_run))
    print("[MODE] resume         =", bool(args.resume))
    print("[MODE] write_only_ok  =", bool(args.write_only_gen_ok))
    ...
    # resume: load existing signatures
    existing_sigs: Set[str] = set()
    if bool(args.resume):
        existing_sigs = load_existing_signatures(out_c)
        if existing_sigs:
            print(f"[RESUME] loaded existing signatures: {len(existing_sigs)} (will skip)")

    # init LLM client (unless dry-run)
    ...
    run_config = {
        ...
        "batch": {
            ...
            "write_only_gen_ok": bool(args.write_only_gen_ok),
            "flush_every": int(args.flush_every),
        },
        "notes": [
            "FULL BATCH VERSION: incremental jsonl append + resume + failure logs + tqdm postfix stats.",
        ],
    }
    ...
    for idx_set, srec in enumerate(pbar, start=1):
        hpos: List[str] = list(srec.get("hpos", []))
        if not hpos:
            continue

        sig = set_signature(hpos)
        ...
        seed_hpo: str = str(srec.get("seed_hpo", hpos[0]))
        ...
        ok = bool(gen_pack.get("ok", False))
        attempts = safe_int(gen_pack.get("attempts", 0), 0)
        attempts_sum += attempts

        dbg = gen_pack.get("debug", {}) or {}
        cov = safe_float(dbg.get("coverage_ratio_valid", dbg.get("coverage_ratio", 0.0)), 0.0)
        if cov > 0.0:
            cov_sum += cov
            cov_n += 1
        ...
        item = {
            ...
            "seed_i": srec.get("seed_i"),
            "seed_hpo": seed_hpo,
            "k": srec.get("k"),
            "hpos": hpos,
            "names": names,
            "clusters": srec.get("clusters"),
            "n_same_cluster": srec.get("n_same_cluster"),
            "n_cross_cluster": srec.get("n_cross_cluster"),
            ...
            "require_full_coverage": bool(int(args.require_full_coverage)),
            "min_coverage_ratio": float(args.min_coverage_ratio),
            "template_lines": gen_pack.get("lines_valid", []),
            "template_raw": (gen_pack.get("raw_text") or "").strip()[:8000],
            "anchors_by_hpo": gen_pack.get("anchors_by_hpo", {}),
            "debug": dbg,
        }
        ...
        if ok:
            n_ok += 1
            if (not bool(args.write_only_gen_ok)) or ok:
                append_jsonl(out_c, item)
                wrote = True
        else:
            n_fail += 1
            # always log failures
            fail_rec = {
                ...
                "names": names,
                "attempts": attempts,
                "coverage_ratio_valid": cov,
                "missing_hpos": dbg.get("missing_hpos", []),
                "n_valid_lines": dbg.get("n_valid_lines", None),
                "min_valid_lines": int(args.min_valid_lines),
                "min_hits_per_line": int(args.min_hits_per_line),
                "raw_preview": (gen_pack.get("raw_text") or "").strip()[:400],
                "per_line_hits_preview": dbg.get("per_line_hits_preview", [])[:3],
            }
            append_jsonl(fail_path, fail_rec)

            if bool(args.debug_print_fail):
                print("\n[FAIL] seed_i=", srec.get("seed_i"), "seed_hpo=", seed_hpo, "k=", srec.get("k"))
                print("  hpos:", hpos)
                print("  missing:", fail_rec.get("missing_hpos"))
                print("  cov:", cov, "valid_lines:", dbg.get("n_valid_lines"), "attempts:", attempts)
                if fail_rec.get("raw_preview"):
                    print("  raw_preview:", fail_rec["raw_preview"].replace("\n", " ")[:240])
        ...
        if int(args.debug_print_every) > 0 and (idx_set % int(args.debug_print_every) == 0):
            print(f"\n[DBG] idx={idx_set} ok={ok} attempts={attempts} cov={cov:.3f} seed={seed_hpo} k={len(hpos)}")

        # tqdm postfix
        elapsed = time.time() - started
        avg_attempts = attempts_sum / max(1, n_seen)
        avg_cov = cov_sum / max(1, cov_n)
        pbar.set_postfix({
            "ok": n_ok,
            "fail": n_fail,
            "skip": n_skipped,
            "wrote": n_written,
            "avg_att": f"{avg_attempts:.2f}",
            "avg_cov": f"{avg_cov:.2f}",
            "sec": int(elapsed),
        })

        if int(args.flush_every) > 0 and (n_seen % int(args.flush_every) == 0):
            print(f"\n[PROG] seen={n_seen} wrote={n_written} ok={n_ok} fail={n_fail} skip={n_skipped}")

    # summary
    avg_attempts = attempts_sum / max(1, n_seen)
    cov_mean = cov_sum / max(1, cov_n)
    summary = {
        "created_at": time.strftime("%Y-%m-%d %H:%M:%S"),
        ...
        "generation_counts": {
            "gen_ok": n_ok,
            "gen_fail": n_fail,
            "avg_attempts_over_seen": float(avg_attempts),
        },
        "coverage_stats": {
            "mean_coverage_ratio_valid": float(cov_mean),
            "n_items_with_cov": int(cov_n),
        },
        "paths": {
            "config": cfg_path,
